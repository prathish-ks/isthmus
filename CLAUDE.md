...
| `src/gateway-approval-coordinator.ts` | Provider-generic credentialed-action approval bridge (approver resolution, delivery, the `pending_approvals` row, expiry, sweep) — every gateway provider's `approvals.subscribe` hands decisions to this one flow, not its own copy |
...
API keys, OAuth tokens, and auth credentials are managed by the OneCLI gateway. Secrets are injected into per-agent containers at request time — none are passed in env vars or through chat context. The container agent sees this via the `onecli-gateway` container skill (`container/skills/onecli-gateway/SKILL.md`), which teaches it how the proxy works, how to handle auth errors, and to never ask for raw credentials. Host-side wiring: `src/gateway-providers/onecli.ts` (the OneCLI `GatewayProviderDefinition`), `src/gateway-approval-coordinator.ts` (the provider-generic approval flow — see below), `ensureAgent()` in `container-runner.ts`. Run `onecli --help`.
...
- **Host-side** (nanoclaw): receives pending approvals and routes them to a human. This logic is **provider-generic**, not OneCLI-specific — `src/gateway-approval-coordinator.ts` owns approver resolution, delivery, the `pending_approvals` row, the card, the click, expiry, and the startup sweep, once for every gateway provider (`docs/gateway-seam.md`). A provider's own module just translates its native protocol into a `GatewayApprovalRequest` and hands it to the coordinator's `decide()` — `src/gateway-providers/onecli.ts`'s `approvals.subscribe` does this by wrapping `onecli.configureManualApproval(cb)` (long-polls `GET /api/approvals/pending`; the SDK exposes no native "ended" event, so `subscribe` adapts the callback into the generic `subscribe(decide, signal): Promise<void>` shape). The coordinator uses `pickApprover` + `pickApprovalDelivery` from `src/modules/approvals/primitive.ts` to DM an approver. Approvers are resolved from the `user_roles` table — preference order: scoped admins for the agent group → global admins → owners. There is no env var like `NANOCLAW_ADMIN_USER_IDS`; roles are persisted in the central DB only. (This generalized out of a OneCLI-only module of the same shape, `onecli-approvals.ts`, in the v2.4.0 promotion's Workstream C7 — extracted behavior-for-behavior, so OneCLI's own approval UX is unchanged; the point was giving a second gateway provider, e.g. Iron Proxy, somewhere to plug in without re-implementing this flow.)
...
- **Bumping `@anthropic-ai/claude-agent-sdk`, `@modelcontextprotocol/sdk`, or any agent-runner runtime dep** → no `minimumReleaseAge` policy applies to this tree. Check the release date on npm, pin deliberately, never `bun update` blindly. The `bun-audit` CI job (mirrors `pnpm-audit`'s baseline pattern — see `container/agent-runner/scripts/check-bun-audit-baseline.ts`) does catch known vulnerabilities in the resulting tree, including transitive ones; it just can't catch a stale-but-not-yet-vulnerable pin the way `minimumReleaseAge` would.
