 ...
 * observable durable facts instead of invisible process state. Read by
 * `container-runner.ts`'s `claimSessionRun` to decide whether a session's
 * existing claim is held by a still-live peer host — v2.4.0 promotion,
 * Workstream C9 (ADR-030): load-bearing for that one read, not shadow state
 * (see `db/coordination.ts`'s own header for the full explanation).
 */
...
async function renewLease(id: string, leaseTtlMs: number): Promise<void> {
  /* eslint-disable no-catch-all/no-catch-all -- a failed renewal must never affect the host; the lease simply expires and the claim becomes takeover-able */
  try {
