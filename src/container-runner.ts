 ...
 *
 * `guard` (EC-02/EC-04, Phase 9) carries `GuardContext` through to
 * `internal/kernel`'s container.kill for the one guarded call site today —
 * the CLI `restart` command's agent-actor branch (`cli/resources/groups.ts`)
 * — so the kernel independently re-verifies the approval that satisfied
 * dispatch()'s own hold, rather than trusting that TypeScript's guard()
 * call having already allowed the replay is enough on its own. Every other
 * caller (an operator-initiated restart, a sweep, self-mod's post-rebuild
 * kill) passes none, matching today's TypeScript, which never gated those
 * kills either.
 */
export function killContainer(sessionId: string, reason: string, onExit?: () => void, guard?: GuardContext): void {
  ...
  void entry.handle.stop(reason, guard).then(
    () => {
...
/**
 * EC-02 (Phase 9): the client `buildAgentGroupImage` uses for
 * container.build_image. A module-level singleton (lazily constructed, like
 * `drivers/index.ts`'s `getSessionDriver`) rather than a fresh `KernelClient`
 * per call — there is nothing per-call to configure, and a shared instance
 * is what `setKernelClientForTests` below can actually replace.
 */
let kernelClient: KernelClientLike | null = null;
function getKernelClient(): KernelClientLike {
  return (kernelClient ??= new KernelClient());
}
/** Test seam: inject a fake, mirroring `drivers/index.ts`'s `resetSessionDriver`. */
export function setKernelClientForTests(client: KernelClientLike | null): void {
  kernelClient = client;
}
 ...
 *
 * EC-02 (Phase 9, ADR-016): the actual `docker build` is delegated to
 * `internal/kernel`'s container.build_image — this is self-mod's
 * install_packages flow's only route to it, and the ONLY production caller
 * of this capability (ADR-016). `guard` carries `SelfModGuardContext` when
 * this runs as an approved self-mod replay (`modules/self-mod/apply.ts`);
 * absent for any other caller (none exist today).
 *
 * The build context directory is no longer this process's own DATA_DIR —
 * the kernel derives it itself from `groupFolder`, never from this caller
 * (see `capability.go`'s `resolveBuildContext`/`GroupFolder` doc comment).
 * The Dockerfile body still never touches disk here: it rides the request
 * payload and is piped to `docker build -f -` inside the kernel.
 */
export async function buildAgentGroupImage(agentGroupId: string, guard?: GuardContext): Promise<void> {
  ...
  // Full docker reference (repo:tag) — `internal/kernel`'s legalTagFragment
  // accepts exactly this shape (one repo:tag pair, each half the same
  // charset a driver realizes labels with).
  const imageTag = `${CONTAINER_IMAGE_BASE}:${agentGroupId}`;

  log.info('Building per-agent-group image', { agentGroupId, imageTag, apt: aptPackages, npm: npmPackages });

  try {
    await getKernelClient().buildImage({
      agentGroupId,
    ...
    throw new Error(`docker build: ${e instanceof Error ? e.message : String(e)}`, { cause: e });
  }
