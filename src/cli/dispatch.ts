...
export async function dispatch(
  ...
  // Populate the grant onto ctx only now, right before the handler runs —
  // never earlier (the guard() call above already consumed opts.grant for
  // its own decision) and never for a fresh, non-replay dispatch. A guarded
  // handler (groups.ts's restart) reads ctx.grant to re-assert the approval
  // to internal/kernel; every other handler ignores the field entirely.
  const handlerCtx: CallerContext = opts.grant
    ? { ...ctx, grant: { approvalId: opts.grant.approval_id, action: opts.grant.action } }
    : ctx;
