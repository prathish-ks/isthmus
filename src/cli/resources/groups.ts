    ...
    restart: {
      ...
      handler: async (args, ctx) => {
        ...
        if (args.rebuild) {
          ...
          // Named, accepted gap (ADR-016): `internal/kernel`'s
          // handleBuildImage only ever consults `checkSelfModGuard` — there
          // is no CLI-restart guard branch for container.build_image — so a
          // `GuardContext` populated here would do nothing at the kernel.
          // This call relies solely on TypeScript's own dispatch()-level
          // guard (the `restart` command's `access: 'approval'` catalog
          // entry, already fully enforced before this handler ever runs),
          // exactly as before EC-02.
        ...
        if (ctx.caller === 'agent') {
          ...
          // EC-02/EC-04 (Phase 9): populate CLIRestartGuardContext only when
          // this handler run IS an approved replay — `ctx.grant` is set by
          // `dispatch.ts` from its own `opts.grant` right before calling
          // this handler, present exactly when a live approval satisfied
          // this command's hold (restart's `access: 'approval'` catalog
          // entry always holds for an agent actor, so an agent-caller
          // restart never reaches this line any other way). Args are
          // reconstructed from what is actually true here rather than
          // re-parsed from the raw request: `id`/`agent_group_id`/`group`
          // all equal `ctx.agentGroupId` for a self-restart, which is what
          // DecideRestartLike's cross-group checks actually key on — the
          // grant's own binding to this exact command name (via
          // grantSatisfies/payload.frame.command) is what really matters
          // for the re-verification, and that comes from `ctx.grant`
          // itself, not from these args.
          const guard: GuardContext | undefined = ctx.grant
            ? {
                cliRestart: {
                  actorKind: 'agent',
                  agentGroupId: ctx.agentGroupId,
                  args: { id, agent_group_id: ctx.agentGroupId, group: ctx.agentGroupId },
                  grant: { approvalId: ctx.grant.approvalId, action: ctx.grant.action },
                },
              }
            : undefined;
          killContainer(
