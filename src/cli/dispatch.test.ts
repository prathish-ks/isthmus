...
describe('CLI scope enforcement', () => {
  ...
  it('approval replay preserves the original agent caller context', async () => {
    ...
    // dispatch.ts populates ctx.grant on an approved replay (right before the
    // handler runs, never earlier) so a guarded handler can re-assert the
    // approval to internal/kernel — every other handler, this one included,
    // just ignores the field. "Preserves the original context" means every
    // OTHER field survives unchanged, not that grant is absent.
    expect(approvalState.observedContexts).toEqual([
      { ...ctx, grant: { approvalId: 'appr-t1', action: 'cli_command' } },
    ]);
    expect(approvalState.requestApproval).toHaveBeenCalledTimes(1);
  });
