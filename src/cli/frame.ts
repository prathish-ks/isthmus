...
/**
 * The verified approval that satisfied this request's hold, when this is an
 * approved replay — populated by dispatch() from its own `opts.grant`
 * (the PendingApproval row `cli.ts`'s socket server passes through on
 * re-entry) right before the handler runs, never by a handler itself.
 *
 * A guarded handler (today: `groups restart`'s agent-actor branch) reads
 * this to populate `internal/kernel`'s `CLIRestartGuardContext.grant`
 * (see EC-02/ADR-015) so the kernel independently re-verifies the grant is
 * still live and names this exact command, rather than trusting that
 * dispatch()'s own guard() call already having allowed the replay is
 * enough on its own. Absent on a fresh (non-replay) dispatch and on every
 * handler that isn't behind a guarded kernel capability — populating it
 * costs nothing there, since checkCLIRestartGuard only consults a
 * GuardContext when the call site itself builds one.
 */
