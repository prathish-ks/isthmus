...
// `createAgent`'s third parameter is `CreateAgentOptions` (a caller-supplied
// options bag — e.g. `suppressCreatedNotify` — unrelated to guarding), not
// `GuardedDeliveryHandler`'s `approval: PendingApproval | null`. createAgent
// never reads the approval, so this thin wrapper — matching
// `GuardedDeliveryHandler`'s shape by taking fewer parameters, same as
// `applyAddMcpServer` above it — is the adapter, rather than overloading
// `createAgent`'s own options slot with two unrelated meanings.
registerDeliveryAction('create_agent', (content, session) => createAgent(content, session), {
