 ...
 * rewrite). This widens Workstream C8's narrow subset (just
 * `modelDomains`/`modelEndpoints`/`inference`, see that workstream's own git
 * history) to the full contract upstream ships, including the mount/file
 * surface (`projectDocument`, `stateVolumes`, `skillBackings`, `skillViews`,
 * `files`) that `container-runner.ts`'s `buildMounts` composes from.
 *
 * Deliberate divergence from upstream, not an oversight: upstream requires
 * `projectDocument` on every registered contract. Here it — and every
 * mount/file field — stays optional, because Isthmus's C8 registrations
 * (`claude.ts` as shipped) declare only model-domain data, no mount surface
 * at all, and must keep compiling and behaving identically while this
 * rewrite lands in stages (see the module-by-module Workstream C14 rows in
 * docs/promotion-v2.4.0.md). `assertProviderHostContractShape` below encodes
 * the actual invariant: a contract either declares NO mount/file surface
 * (buildMounts keeps composing that provider's mounts the legacy way) or a
 * COMPLETE one, including `projectDocument` — there is no valid in-between
 * where e.g. a state volume exists with no project document to carry it.
 */
...
export interface ProviderHostContract {
  ...
  /**
   * Core-composed project document carrying the provider's standing
   * instructions. Isthmus divergence from upstream (there this is
   * required): undeclared means this provider has not opted into the
   * mount-composition rewrite — see this file's own header.
   */
...
/** Whether this provider's registered contract declares a mount/file surface (opted into the C14 rewrite). */
export function hasProviderMountSurface(name: string | null | undefined): boolean {
  return getProviderHostContract(name)?.projectDocument !== undefined;
}
...
export function assertProviderHostContractShape(provider: string, contract: ProviderHostContract): void {
  ...
  const skillViews = contract.skillViews ?? [];
  const files = contract.files ?? [];
  for (const [field, value] of [
    ['stateVolumes', stateVolumes],
    ['skillBackings', skillBackings],
    ['skillViews', skillViews],
    ['files', files],
  ] as const) {
    assertArray(value, `${provider}.${field}`);
  }
  ...
  // Isthmus divergence (see this file's own header): a contract with no
  // mount/file surface at all — Isthmus's own C8 registrations, as shipped —
  // is valid and stops here. buildMounts keeps composing that provider's
  // mounts the legacy way. Any mount/file field present at all commits the
  // contract to the full shape upstream requires, projectDocument included.
  const hasMountSurface =
    contract.projectDocument !== undefined ||
    stateVolumes.length > 0 ||
    skillBackings.length > 0 ||
    skillViews.length > 0 ||
    files.length > 0;
  if (!hasMountSurface) return;
  if (contract.projectDocument === undefined) {
    throw new Error(`${provider}.projectDocument is required once any mount/file surface is declared`);
  }

  const volumeIds = unique(
    stateVolumes.map((volume) => volume.id),
    `${provider}.stateVolumes[].id`,
  );
  const backingIds = unique(
    skillBackings.map((backing) => backing.id),
  ...
  for (const file of files) {
    ...
    const volume = stateVolumes.find((candidate) => candidate.id === file.volumeId)!;
