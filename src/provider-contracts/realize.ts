 ...
 * rewrite), step 2/6.
 *
 * Deliberate divergence from upstream, not an oversight: upstream imports
 * `writeAtomic` from `migrate-claude-memory-settings.ts`, which is safe
 * there only because that file runs once at startup, before any container
 * exists to race against. This module's file writes/reconciliation land in
 * the same agent-writable provider state-volume tree `project-doc-compose.ts`'s
 * own `writeAtomic` was written to protect (LOAD-BEARING comment on that
 * function) — group-scoped state volumes are shared across a group's
 * sessions and mounted read-write into every one of them, so a later
 * group-init pass reconciling an existing file can race a live container's
 * write access to the same directory. Reuses that function instead.
 */
import fs from 'fs';
...
import { BASE_INSTRUCTIONS_PATH, writeAtomic, type ProjectDocSpec } from '../project-doc-compose.js';
