 ...
 *
 * v2.4.0 promotion, Workstream C14: added for the provider-host-contract
 * mount-composition rewrite. `DEFAULT_PROJECT_DOC` deliberately keeps using
 * the legacy `baseDocPath` shape below rather than switching to this one —
...
export interface ProjectDocSpec {
  ...
  /**
   * Shared base document, relative to the project root. A missing file is
   * not an error. Legacy shape: a spec that sets this renders the base
   * document as-is, ignoring `instructions`. New contracts should leave this
   * unset and use `instructions` instead — see `BASE_INSTRUCTIONS_PATH`.
   */
  baseDocPath?: string;
  /**
 ...
 * A template without the placeholder (e.g. Isthmus's current
 * `container/CLAUDE.md`) renders unchanged — `String.replace` is a no-op
 * when the target text isn't present.
 */
...
export async function composeGroupProjectDoc(
  ...
  // Legacy spec (baseDocPath set): renders the base document as-is, ignoring
  // `instructions` — matches every existing spec (DEFAULT_PROJECT_DOC), zero
