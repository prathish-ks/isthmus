...
export interface SessionHandle {
   ...
   */
  /**
   * `guard` (EC-02/EC-04, Phase 9) carries the CLI-restart guard context
   * for the ONE call site that is guarded today — `groups.ts`'s restart
   * handler, agent-actor branch. Optional and additive: every other caller
   * (an operator-initiated restart, a sweep, self-mod's post-rebuild kill)
   * passes no second argument, and a driver that has not been updated to
   * read it (there is only one realization in this tree) still satisfies
   * this interface — TypeScript's rule that a function accepting fewer
   * parameters is assignable to a type expecting more.
   */
  stop(reason: string, guard?: GuardContext): Promise<void>;
  /**
