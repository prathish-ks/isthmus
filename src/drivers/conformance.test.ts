...
function dockerHarness(): Harness {
  ...
  // EC-02 (Phase 9, ADR-016) moved the actual `docker create`/`docker start`
  // behind `internal/kernel` — this driver no longer shells them via `cli`
  // at all (see docker-driver.test.ts's "never calls docker create
  // directly"), so a fake kernel client is injected the same way `FakeCli`
  // already is, and `realize()` below reconstructs what was realized from
  // what the driver handed the kernel client rather than from a `docker
  // create` argv that no longer exists.
  ...
  const driver = withSessionEvents(new DockerSessionDriver({ ...FIXTURE_POLICY, cli, kernelClient }));
  return {
    name: 'docker',
    driver,
    cli,
    ...
    async realize(spec) {
      ...
      // `SessionSpec` — `KernelClient` does the wire translation, not this
      // driver), not what a real kernel would derive from it.
      const { spec: sentSpec } = kernelClient.wakeCalls.at(-1)!;
      const agent = sentSpec.containers.find((c) => c.role === 'agent')!;
      const mounts = agent.mounts.map((m) => ({
        hostPath: m.hostPath,
        containerPath: m.containerPath,
        ro: m.mode === 'ro',
      }));
      // The composed and contributed lanes are carried to the kernel
      // separately (never merged here — see docker-driver.test.ts's "carries
      // composed env and contributed env as separate lanes"); the kernel
      // applies last-wins ordering itself (exec.go). Merging the same way
      // here is what lets this harness observe the one merged view a real
      // container's environment would actually have.
      const env = { ...agent.env, ...agent.contributedEnv };
      // The four canonical labels are derived by the kernel itself
      // (naming.go's LabelsForKey), not carried on the TS-side spec —
      // `labelsForKey` is the same pure port `docker-driver.ts` used to
      // stamp them before EC-02, kept here purely to reconstruct the
      // realized view.
      const labels = { ...labelsForKey(sentSpec.key, 'agent'), ...sentSpec.labels, ...agent.labels };
      return { containers: [{ role: 'agent', image: agent.image, env, mounts }], labels };
    },
    failWith(message) {
      // A real kernel-mediated wake failure arrives as a `KernelError` whose
      // `detail` carries the raw `docker create`/`docker build` stderr text
      // — `translateKernelError`'s `exec-failed` branch re-runs the same
      // message-pattern classifier a direct-CLI failure always went through
      // (see docker-driver.ts). A plain (non-`KernelError`) rejection would
      // instead mean "the kernel process itself is unreachable", which is a
      // different, always-`runtime-unavailable` failure this harness does
      // not use.
      kernelClient.wakeError = new KernelError({ code: 'exec-failed', detail: message });
    },
  };
}
...
describe('conformance: lifecycle', () => {
  ...
  eachDriver('stop is full teardown and reports no terminal event', async (h) => {
    ...
    // EC-02 (Phase 9, ADR-016) moved the actual stop-then-remove behind
    // `internal/kernel`: on the happy path (the kernel still has a record of
    // this session — the only path this fixture exercises) `stop()` issues
    // no local `docker stop`/`docker rm` at all (see docker-driver.test.ts's
    // "kills via the kernel client"). Whatever the runtime calls it,
    // nothing this key allocated may survive — that guarantee now lives in
    // having asked the kernel to tear the session down.
    expect(h.kernelClient.killCalls).toEqual([{ sessionId: 's1', reason: 'host-shutdown', guard: undefined }]);
  });
