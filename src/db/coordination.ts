 ...
 * `delivery_attempts`, and `wake_signals`.
 *
 * v2.4.0 promotion, Workstream C9 (ADR-030): `host_instances` and
 * `session_claims` are load-bearing, not shadow — `container-runner.ts`'s
 * `claimSessionRun` genuinely refuses a spawn when a live peer host holds the
 * claim (see that function's own doc comment). `delivery_attempts` and
 * `wake_signals` remain unconsumed by anything in this tree and stay shadow
 * surface, matching their own upstream status.
 *
...
/**
 * Run a shadow write (`delivery_attempts`/`wake_signals` only — see this
 * file's own header). Failures log and are swallowed: shadow state must
