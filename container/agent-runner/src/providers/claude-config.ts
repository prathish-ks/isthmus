 ...
 *
 * v2.4.0 promotion, Workstream C15: ported from upstream's rewrite, with one
 * deliberate divergence — see `TOOL_ALLOWLIST`'s own comment for why
 * `'TaskOutput'` stays in this fork's allowlist though upstream's v2.4.0
 * dropped it.
 */
...
//
// v2.4.0 promotion, Workstream C15 divergence, not an oversight: upstream's
// own v2.4.0 rewrite of this list DROPS 'TaskOutput' (confirmed directly —
// zero occurrences anywhere in v2.4.0's container tree, `git grep` against
// the real tag). This fork's list had 'TaskOutput' since its very first
// commit (present at the pinned v2.3.0 baseline too), and there is no
// evidence the removal was a deliberate, reasoned upstream decision rather
// than an oversight during the refactor — dropping it here would silently
// take away a working SDK tool (subagent output retrieval) from every
// Claude session. Preserved.
