 ...
 *
 * v2.4.0 promotion, Workstream C15: registration is now two-step and
 * order-independent — the provider module registers its factory
 * (`registerProvider`), and the contract module attaches the runtime
 * contract (`registerProviderContract`). The two files never import each
 * other, so a payload's provider module still compiles on a core that
 * predates the contract seam — the contract file simply has nothing to
 * attach to there and is not imported. Every provider, Claude included,
