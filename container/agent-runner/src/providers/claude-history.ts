 ...
 *
 * v2.4.0 promotion, Workstream C15: ported near-verbatim, extracted from the
 * pre-C15 `providers/claude.ts` (same 2000-char truncation, same date
 * formatting, same rotation thresholds/env vars — behaviorally equivalent,
 * not a rewrite) plus `resolveContainedPath`, a real path-traversal guard
 * upstream added that the pre-C15 version didn't have. Adopted: the archive
 * path was already always safe in practice (the summary-derived name is
 * regex-sanitized before use), so this is pure defense-in-depth, not a
 * behavior change.
 */
