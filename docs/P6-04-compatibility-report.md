...
**Why this doesn't count against P6-03's "zero Go edit" claim for the same customization (catalogue scenario #17):** this was a one-time architecture-parity fix, porting a fix already made once, for real, on the TS side. It was not a Go change *caused by* adding a new provider-mount customization. After this fix, adding a new provider that follows the identical `origin: provider` contract (any future gateway or model-provider skill) requires zero further Go changes — exactly what P6-03 demonstrated, and what this report confirms empirically rather than by assertion.
