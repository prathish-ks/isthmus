...
**Approved as accepted.** This is a strictly stricter failure mode (reject-and-skip on ambiguous input, never silently accept malformed input as done in the lenient parser), which is the correct direction of divergence for a hardened kernel. No fixture in the 20-fixture scope exercises a non-canonical timestamp, so it is unexercised by any parity test but poses no known behavioral risk.
...
**Reconstruct a composed `routeInbound` equivalent in Go now, to close the `lastActiveTouched`/`containerWake.attempted` gaps.** Considered and rejected: doing so would mean writing new production-shaped orchestration code not requested by any P4-0x task, guessing at composition details from snapshot shape rather than real source in at least one case (the mock-leakage artifact has no correct Go equivalent to write at all), and expanding this task's 3-hour estimate into something closer to a full router port — itself a future task, if ever undertaken, given `router.ts` is currently classified BOUNDARY/KEEP-TYPESCRIPT.
