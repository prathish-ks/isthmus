...
| 6 | Customization regression | Manual — the 20-scenario catalogue (P6-01) and preservation gate (P6-05) are a checklist/report, not a CI job | **Re-run 2026-09-20 against this release candidate (`a34ec854`, includes PR #13's kernel-socket lock): still 20/20 (100%).** Method: rather than re-executing all 20 scenarios live (disproportionate for a routine re-check with no customization-surface changes to investigate), audited `git log` for every Go package the catalogue's 20 verdicts cite (`internal/mount`, `internal/containerdefaults`, `internal/kernel`, `internal/routing`, `internal/credential`) back to the original import — all five are untouched since Phase 6 except `internal/kernel`, where the only change (PR #13's `acquireServeLock`) is confirmed isolated to `Serve`'s pre-`Listen` startup sequence and new test/lock files, touching neither `Dispatch` nor any `route.request`/`capability.request` handler the 20 scenarios route through. | **Closed for this release candidate.** Re-run this audit (or a full live re-execution, if a future change actually touches one of the cited packages) before any future tag once TS customization surface or these Go packages change. |
