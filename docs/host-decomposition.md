...
- Caveat: the *decide* functions themselves (defined per-action in `permissions/guard.ts`, `agent-to-agent`, etc.) are policy, not kernel — they stay in TypeScript modules and get *invoked through* the guard, not absorbed into it. Only the choke point itself is a kernel candidate.
