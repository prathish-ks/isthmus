...
**Update, 2026-09-20 — both root-caused and resolved on this fork.** A coverage-uplift exercise cross-checked this repo's test results against a clean upstream `v2.3.0` worktree (a full `vitest run` on each, disk headroom controlled for — the first attempt at this comparison was confounded by low free disk on the dev machine, which independently trips `transaction.ts`'s own 256MB snapshot-space guard and looked like extra failures until re-run with disk freed). Neither of the two files above still fails on this fork:

- **`scripts/update/transaction.e2e.test.ts`**: the 4 failures reproduce identically on a clean, unmodified `v2.3.0` checkout (confirmed directly), so this was always a real upstream bug, not environment noise. Root cause: `hasSafeStatePaths`/`loadState` in `scripts/update/transaction.ts` compare paths with plain `path.resolve()`, which does not dereference symlinks. `os.tmpdir()` on macOS resolves through `/var` → `/private/var`, so a state file's persisted (already-realpath'd) `projectRoot` never matches a caller's raw, non-realpath'd path, and the safety check spuriously rejects legitimate state. Isthmus's `transaction.ts` (introduced with the Go-kernel work) already fixes this with a symlink-aware `realResolve()` helper used consistently in both functions — confirmed green (7/7, including 3 additional tests this fork added beyond upstream's 4) across repeated runs.
- **`scripts/add-dial-tool-scope.test.ts`**: no longer reproduces (16/16 passing, repeated runs). The suite shells out only to a fully self-contained fake `onecli` script (a `jq`-driven shim placed on `PATH` for the test) plus real `jq` — no live network, Docker, or OneCLI dependency — so the original "subprocess exiting non-zero" was specific to the August 2026 recording machine's environment (most plausibly a missing or different `jq`), not a code defect.
