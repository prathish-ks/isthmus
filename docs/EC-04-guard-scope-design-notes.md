...
The real `restart` command (`src/cli/resources/groups.ts`, confirmed by direct read) is `{ resource: 'groups', access: 'approval' }`, with no `hostOnly` — exactly matching the fixture's `restartLikeCmd` synthetic stand-in, which is why the fixture is a faithful golden baseline for it, not an approximation.
