...
|---|---|---|
| Channel → kernel wake (`wakeContainer` via `router.ts`) | **Spans the seam** | `src/cli-channel-kernel-smoke.test.ts` (ADR-022). Extended for the v2.4.0 promotion (Workstream E3, 2026-09-26) to also pin `SessionSpec.networkAccess`'s exact wire shape across this same real `composeSessionSpec` → `toWireSession` → real-socket path — the new gateway-trust/multi-container field Workstream A/B added, verified with a negative control |
...
section. The v2.4.0 promotion's own security-review pass (Workstream C6,
2026-09-25) applied the same discipline to the gateway-provider surface this
promotion adds and found three more real, previously-unverified boundary
claims — all three fixed and closed the same day, not merely disclosed:
...
| A gateway-composed `ContainerSpec.labels` can never override the four canonical adoption labels (`nanoclaw-install/-group/-session/-role`) that session-adoption logic trusts as ground truth | **Fixed and verified** (v2.4.0 promotion, Workstream C6 Finding 1, closed 2026-09-25) | `go-host/internal/kernel/naming_test.go` — `LabelsForKey` applies the four canonical labels last, unconditionally; found via direct trace-through (not doc-comment trust) that `mount.ValidateSpec` does not validate `ContainerSpec.labels` by design, making this a real cross-install impersonation vector the moment a gateway populates `.containers[].labels` (latent at the time — neither OneCLI nor Iron Proxy did yet). Verified via `git stash`-based negative control |
| A gateway-named `approverUserId` (the one verified channel identity a gateway policy selected) is actually persisted and consulted, not silently widened to "any admin for the group decides" | **Fixed and verified** (v2.4.0 promotion, Workstream C6 Finding 2, closed 2026-09-25) | `src/gateway-approval-coordinator.default-approval.test.ts` (+ coverage in `guard.test.ts`/`message-gate.test.ts`/`approvals/primitive.coverage.test.ts`) — `request.approverUserId` now threads through to the persisted row; deliberately `?? null`, not `?? resolvedTarget.userId` (the latter would have wrongly over-narrowed the common unnamed-approver case — caught by tracing `resolvedTarget`'s own construction before trusting the reviewing agent's first suggested fix) |
| OneCLI's `allowlisted-extra`-class provider mounts restrict *which* host paths a `-v` mount can name, not just the argv grammar around it | **Fixed and verified** (v2.4.0 promotion, Workstream C6 Finding 3, closed 2026-09-25, explicit founder direction) | `src/gateway-providers/onecli.test.ts` — `contributionFromArgs` now refuses any `-v` mount whose resolved host path isn't one of the three fixed locations the real `@onecli-sh/sdk` source was read to confirm it actually writes to (`os.tmpdir()`-rooted CA/stub files); before this fix, a compromised/buggy SDK or remote OneCLI service could have returned a mount reaching arbitrary host paths (e.g. `~/.ssh`), admitted read-write with no further check. New suite covers a disallowed path, near-miss sibling/wrong-prefix/traversal attempts, and the legitimate paths; verified via `git stash`-based negative control |
...
| [029](../go-host/docs/ADR-029-gateway-session-lifecycle-adoption.md) | v2.4.0 promotion, Workstream C0: initial gateway-session-lifecycle adoption decision. | 2026-09-25 | **Superseded same day** by [030](../go-host/docs/ADR-030-gateway-adoption-and-multi-host-coordination.md) — two factual errors found after recording (the approval subsystem is not duplicative; a second error corrected in the successor) |
| [030](../go-host/docs/ADR-030-gateway-adoption-and-multi-host-coordination.md) | v2.4.0 promotion: gateway adoption and multi-host coordination — supersedes 029 entirely (recorded as a new ADR, not an edit, to keep decision history honest). Scopes what's kernel-enforced vs. TS-accepted for the gateway-provider trust boundary; gates Workstream C's C1/C2. | 2026-09-25 | Compliant citation (LAW-07/08 — trust-boundary scope, the same role ADR-016 plays for LAW-07 generally) |
| [031](../go-host/docs/ADR-031-community-portal-adoption.md) | v2.4.0 promotion: adopts the community-portal feature, reversing Workstream C4's original "do not adopt" recommendation, by direct founder direction. Adds Workstream C tasks C10/C11. | 2026-09-25 | N/A (feature-adoption decision, not a law-scoped one) |
| [032](../go-host/docs/ADR-032-provider-contract-rewrites.md) | v2.4.0 promotion: provider host-contract (C14, host-side mount/file composition) and runtime-contract (C15, container-side execution-policy/inference/memory/command-formatting) rewrites — both characterized with real tests before/through the rewrite, per LAW-06. | 2026-09-26 | Compliant citation (LAW-06 — contracts before rewrites) |
| [033](../go-host/docs/ADR-033-egress-lockdown-gateway-generalization.md) | v2.4.0 promotion: generalizes egress-lockdown's gateway attachment from hardcoded OneCLI to any configured gateway, adapted to Isthmus's own kernel-startup-scoped enforcement model (ADR-024/025/026) rather than upstream's per-session one, which doesn't apply here. | 2026-09-26 | Compliant citation (LAW-07/08 — closes a real "silently mis-targets the wrong gateway" gap the moment a second gateway becomes default) |
| [034](../go-host/docs/ADR-034-go-kernel-mount-network-model.md) | v2.4.0 promotion: records Workstream A's Go-kernel mount/network model additions (gateway-trust class, multi-container network isolation) and the mixed-version compatibility matrix for the `ProtocolVersion` v1->v2 bump. | 2026-09-26 | Compliant citation (LAW-06/09 — contracts before rewrites; upstream moves independently) |
