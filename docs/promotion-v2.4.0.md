...
| D4 | Confirm zero rows remain unclassified and zero Bucket B rows lack a security-review status before this workstream counts as done — this is also a promotion-gate line, not just an internal target | **Blocked on exactly one thing**: `src/modules/community-portal/*` (4 rows) is the only Bucket B row without a `security_review_status`. Every other row in the 526-row inventory is fully classified with zero gaps. This is now a real decision, not busywork — see the changelog entry for the finding and the founder question it raises |
