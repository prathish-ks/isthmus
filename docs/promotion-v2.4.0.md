...
| # | Task | Status |
|---|---|---|
| H1 | Resolve the open question above: identify or confirm the actual current onboarding path from a plain nanocoai/nanoclaw install to Isthmus | **Done — confirmed by D2's direct read of both candidate skills: no such path exists today.** `migrate-nanoclaw` and `update-nanoclaw` both operate exclusively within the `nanocoai/nanoclaw` git lineage itself (resolve `upstream`/`origin` to that exact remote, stamp the finish condition against that remote's own commit/tree) — neither has any mechanism for landing on a separately-maintained downstream project. Recorded as a real finding, not papered over, per the Migration Continuity section's own instruction. **This is a decision point, not an implementation gap I'm closing unilaterally**: building a bespoke nanoclaw→Isthmus onboarding path is a real design/build effort (which source versions to support, whether it reuses `migrate-nanoclaw`'s methodology as D2 suggests, how deep the Go-kernel layer's own onboarding needs to be) — flagging for the founder rather than designing and building a new migration mechanism under the standing autonomous-execution latitude, the same bar C8's `provider-contracts`/`setup/gateways/` blocker and C4's `community-portal` finding were already held to |
| H2 | **Acceptance test, source = nanoclaw v2.3.0**: starting from a clean plain nanoclaw v2.3.0 install, run the identified path end to end and verify all of: existing user data and configuration preserved; credentials are not copied into any location the kernel wouldn't admit as a valid mount; groups, sessions, mounts, and central DB state remain valid after migration; the resulting installation is running Isthmus pinned to the new v2.4.0 baseline; a deliberately-induced failure partway through leaves a recoverable state (not partial/corrupt); rollback behavior is documented **and produces the recorded rollback artifact above**; the whole procedure runs from a fresh checkout with no reliance on undocumented local state | **Blocked on H1's finding** — there is no identified path to run this acceptance test against yet. Waiting on the founder decision H1 flags (whether/how to build a bespoke onboarding path) before this can move |
| H3 | **Acceptance test, source = nanoclaw v2.4.0**: the same full checklist as H2, including its own rollback artifact, against a clean plain nanoclaw v2.4.0 install instead | **Blocked on H1's finding**, same reason as H2 |
