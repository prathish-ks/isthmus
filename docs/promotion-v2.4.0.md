...
| C8 | **New (ADR-030), sequence third.** Install `/add-iron-proxy` on par with upstream v2.4.0's own catalog: the provider payload, the approval-bridge middleware, the local Docker-built proxy + Iron Control console, `NANOCLAW_IRON_PROXY_PORT`/`NANOCLAW_IRON_CONTROL_PORT` wiring, the gRPC bridge dependencies. First real consumer of Workstream A's `gateway-trust` mount class and multi-container/`networkAccess` executor | **Partially done, genuinely blocked on the rest.** All 38 skill files copied verbatim from upstream v2.4.0 into `.claude/skills/add-iron-proxy/` (commit `c2b26b83`, `feat/gateway-provider-seam`) — `gateway.json`'s `"default": false` correctly keeps OneCLI the active gateway; nothing forces Iron Proxy on. This satisfies "catalogued, matching upstream" — the skill is present and directly invokable (`/add-iron-proxy`, Isthmus's own convention for every other `/add-*` skill; Isthmus has no generic gateway-picker UI to register with, unlike upstream's `setup/gateways/catalog.ts`, and doesn't need one for this). **`src/provider-contracts/` resolved (commit `7d45ffc5`) — narrow, not the full port.** Checked exactly what the skill's 7 dependent files actually call: only `getProviderModelEndpoint`/`providerModelAllowedHosts` (`iron-proxy.ts`, `auth.ts`, `setup.ts`, `credential-store.ts`) and, separately, `inference.speedTiers` (the `--speed` flag, not wired). None of it touches `stateVolumes`/`skillBackings`/`skillViews`/`files` — the mount/file-composition half that would've meant rewriting `buildMounts` for Claude/Codex/OpenCode too. Ported only the used slice: `src/provider-contracts/registry.ts` (a static per-provider model-domain/endpoint registry, field names matching upstream's `ProviderHostContract` for painless extension later), `claude.ts` (Claude's own registration: `api.anthropic.com`), wired as a side-effect-import barrel exactly like `providers/index.ts`. This also let `gateway-approval-coordinator.ts` (C7) gain the default-model-traffic auto-approval upstream's own design has and C7 shipped without — a `'default'`-trigger request to a declared model domain now auto-approves instead of asking a human for every ordinary API call; `'policy'`-trigger and missing-trigger requests are never auto-approved, whatever the destination. OneCLI's own requests are always `trigger: 'policy'` (confirmed in `onecli.ts`), so this path is unreachable through OneCLI today — it's there for when a `'default'`-trigger gateway (Iron Proxy) exists. Tested via a stub `GatewayProviderDefinition` driving `decide()` directly (OneCLI can't reach this branch itself): match/no-match/wrong-trigger/missing-trigger, all passing. `pnpm exec tsc --noEmit` clean; full `pnpm test` green (3999 tests, up from 3987); `eslint` clean except pre-existing warnings.
...
| D3 | For each Bucket C row: is it a genuinely independent upstream change (safe to port as-is), or does it touch a file Isthmus has already meaningfully modified (needs manual reconciliation, feature-by-feature, preserving both sides)? Record the decision in the inventory's `reconciliation_decision` column | **Partially done as a byproduct of D1's per-file reading** — every Bucket C row already carries a `reconciliation_decision` (`port-verbatim` / `port-with-modification` / `declined:<reason>`) from the classification pass itself, since the agents were instructed to read the actual diff rather than guess. What remains is *executing* the `port-with-modification` rows that need real reconciliation against Isthmus's own divergence — named explicitly: the Mattermost managed-install-vs-guidance-only conflict, the egress-lockdown auxiliary-gateway generalization (active branch), and the reconcile-session stuck-detection feature if adopted. **Mattermost investigated 2026-09-26, deliberately not executed — real evidence of unrelated concurrent/parked work on this exact feature, not a simple reconciliation.** The CSV's own `port-with-modification` rows (`setup/channels/mattermost-{config,discovery,guidance,response,runtime-verification}.test.ts`, two fixture files) turned out to be inconsistent with a sibling group of Mattermost rows already correctly marked `declined:out-of-scope-tracked-by-sync-sibling-branch-mechanism` (`.claude/skills/add-mattermost/*`) — same feature, two different dispositions recorded for it. Checked why directly: `setup/channels/mattermost-guidance.test.ts` does not exist at the `v2.3.0` tag at all (confirmed via `git show v2.3.0:<path>`), yet is present in Isthmus's very first commit ("Isthmus v1: a small Go trust-kernel for NanoClaw") — meaning Isthmus's fork point already carried Mattermost setup content ahead of the v2.3.0 tag, not something this promotion's v2.3.0→v2.4.0 diff introduced cleanly. `git log --all` on this file surfaces a separate local branch, `fix/inbox-toctou-batch-revalidation`, with five Mattermost-specific commits (last dated 2026-09-23, three days before this check — parked, not live, but real and unmerged) plus PR-numbered commit messages suggesting upstream-sourced content, not Isthmus-authored. Given this shared checkout already carries other concurrent/parked work (this session's own earlier stash-recovery incidents), reconciling Mattermost here risks conflicting with whatever that branch represents rather than genuinely resolving a v2.3.0→v2.4.0 diff question. **Done, the safe half**: corrected the CSV's inconsistency directly — all 7 `port-with-modification` Mattermost rows (5 test files, 2 fixtures) reclassified to `declined:out-of-scope-tracked-by-sync-sibling-branch-mechanism`, matching the 11 `.claude/skills/add-mattermost/*` rows that already carried the correct disposition; all 18 Mattermost rows now agree. **Not executed, and shouldn't be without founder input**: no code reconciliation — the founder (or whoever owns `fix/inbox-toctou-batch-revalidation`) should clarify whether that branch is meant to land before or instead of any Mattermost work this promotion does. Egress-lockdown auxiliary-gateway generalization and the reconcile-session cluster remain not started, for the same active-branch-coordination reason named when D1 first found them. |
...
| G3 | For any new script/automation this promotion introduces (e.g. a migration-continuity check, Workstream H): add `sync-sibling-branch-script-test`-style regression coverage, run on every PR — required, not report-only, unless there's a specific reason it can't be (state the reason if so) | **Not yet applicable — blocked on H2/H3, not merely unstarted.** Checked: this promotion has not introduced any new standalone script/automation so far — every Workstream C/D14 change modified an existing library/registry file, covered by the ordinary `test` job (see G2). The one place a new script is expected is Workstream H's migration-continuity acceptance tests (H2/H3), not yet started; this task genuinely has nothing to cover until that lands |
...
| H2 | **Acceptance test, source = nanoclaw v2.3.0**: starting from a clean plain nanoclaw v2.3.0 install, run the identified path end to end and verify all of: existing user data and configuration preserved; credentials are not copied into any location the kernel wouldn't admit as a valid mount; groups, sessions, mounts, and central DB state remain valid after migration; the resulting installation is running Isthmus pinned to the new v2.4.0 baseline; a deliberately-induced failure partway through leaves a recoverable state (not partial/corrupt); rollback behavior is documented **and produces the recorded rollback artifact above**; the whole procedure runs from a fresh checkout with no reliance on undocumented local state | **Unblocked by H1's correction.** The mechanism (`isthmus.sh`) exists and is dry-run-proven in general; this row is now specifically "run it starting from a nanoclaw v2.3.0 source and confirm the result is pinned to *this promotion's* new v2.4.0 baseline, with a recorded rollback artifact" — narrower than the earlier "design a path" framing. Not started |
| H3 | **Acceptance test, source = nanoclaw v2.4.0**: the same full checklist as H2, including its own rollback artifact, against a clean plain nanoclaw v2.4.0 install instead | **Unblocked by H1's correction**, same reason as H2. Not started |
| H4 | Confirm both H2 and H3 land the user on the identical resulting state (same pinned baseline, same expected behavior) — a v2.3.0-sourced and a v2.4.0-sourced migration are not allowed to diverge in outcome | Blocked on H2/H3 |
