...
| D1 | Classify every row into Bucket A/B/C using the inventory — supersedes the earlier prose-based first pass | **In progress, 280 of 526 rows classified (2026-09-25, this session).** Beyond the original 116 (Workstream B's 18 + the 98 Non-goals-text sibling-branch/Iron-Proxy rows): `container/agent-runner/src/provider-contracts` + `providers` (33) and `src/provider-contracts`'s unported remainder (7) tied to Workstream B's already-flagged, still-deferred provider-host-contract question; `src/provider-contracts/registry.ts`+`claude.ts` (2) and `src/gateway-providers/{index,installed}.ts`+2 tests (4) as C8/C7 bookkeeping catch-up; `.claude/skills/add-onecli/*` (21) as declined-by-design (C7 kept OneCLI core-baked); `setup/gateways/*` (12) as C8's own already-documented not-yet-built gap; `.claude/skills/slack-agent-flow/`+`slack-a2a-rooms/` (30) as sibling-branch-tracked — this one verified directly against `.github/workflows/ci.yml`'s `sync-sibling-branches` job comment (mirrors the whole `channels` branch generically, not a per-skill allowlist) rather than inferred by naming pattern, which is exactly the caution this row's earlier text asked for; 8 small Isthmus-maintained skill files flagged for D3, `init-onecli`'s full upstream deletion (308 lines) recorded as a real, separate finding, not acted on; and every file this session directly authored under C7/C9 (7 files) and C10/C11 (8 files, `src/community-portal/*`'s 19 rows already covered by C11's own commit) as adopted bookkeeping catch-up. **246 rows remained after that pass** — dispatched to three parallel background agents (`container/agent-runner`+`src/modules` — 75; `setup/*` remainder — 77; `src/db`/`cli`/`channels`/`providers`/`mailbox`/`drivers` + the long tail of individual `src/*.ts` root files — 94), each instructed to read every file's actual v2.3.0→v2.4.0 diff (not guess from the filename), apply this plan's own established precedents (the inert `wakeContainer`→`requestWake` rename, the shadow-write durable-coordination pattern, the deferred provider-host-contract question), and flag rather than silently classify anything Bucket-B-shaped or anything that looked like already-adopted C7/C9/C10/C11 work slipping through. Results pending merge as of this entry |
