...
export async function run(_args: string[]): Promise<void> {
  ...
  if (fs.existsSync(envFile)) {
    ...
    // NANOCLAW_GATEWAY_PROVIDER added alongside ONECLI_URL (v2.4.0
    // promotion), not replacing it: upstream swapped the two because they
    // moved OneCLI out of trunk, making a gateway provider always-explicit.
    // Isthmus kept OneCLI core-baked and implicit-by-default (unset means
    // OneCLI, per gateway-providers/index.ts's own configuredGatewayProviderKind
    // doc comment: "an install that never sets it behaves exactly as it
    // always has") — a correctly-configured install may have neither env var
    // set at all, so ONECLI_URL stays a valid signal here, not just an
    // explicit NANOCLAW_GATEWAY_PROVIDER override.
    if (
      /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|ANTHROPIC_AUTH_TOKEN|ONECLI_URL|NANOCLAW_GATEWAY_PROVIDER)=/m.test(
        envContent,
      )
    ) {
