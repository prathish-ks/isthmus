...
vi.mock('@clack/prompts', () => ({
  ...
  // v2.4.0 promotion, Workstream C11: `error` added. Isthmus's
  // setup/lib/runner.ts (run-channel-skill.ts's failure path) calls
  // `p.log.error`, which the upstream test's own mock never covered —
  // this fixture only needed to grow, not narrow, to fit Isthmus's own
  // channel-skill runner underneath the ported portal.ts.
  log: { info: mock.logs, success: mock.logs, warn: mock.logs, step: mock.logs, message: mock.logs, error: mock.logs },
}));
